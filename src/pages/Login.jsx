...
const Login = () => {
  const { axios, loginData, setLoginData, setUser, setIsAuthenticated } = useContext(AppContext);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data.success) { // res holo akta obj jar key: data,status, header, config...ei data key te user info thake
          //   res = {
          //   data: {
          //     success: true,
          //     user: { name: "Rahim", email: "rahim@gmail.com" },
    ...
    <div className="min-h-screen flex justify-center items-center bg-gray-100">
      <form onSubmit={handleSubmit} className="bg-white text-gray-500 max-w-[340px] w-full mx-4 md:p-6 p-4 py-8 text-left text-sm rounded-xl shadow-[0px_0px_10px_0px] shadow-black/10">
        ...
        <div className="flex items-center my-2 border bg-indigo-500/5 border-gray-500/10 rounded gap-1 pl-2">
          ...
          <input className="w-full outline-none bg-transparent py-2.5" type="email" placeholder="Email" name="email" value={loginData.email} onChange={handleChange} required
          />
        </div>
        ...
        <div className="flex items-center mt-2 mb-4 border bg-indigo-500/5 border-gray-500/10 rounded gap-2 pl-2">
          ...
          <input className="w-full outline-none bg-transparent py-2.5" type="password" placeholder="Password" name="password" value={loginData.password} onChange={handleChange} required/>
        </div>
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <a className="text-blue-600 underline text-sm" href="#">Forgot Password?</a>
        </div>
